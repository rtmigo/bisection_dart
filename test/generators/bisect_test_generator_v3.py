...
if __name__ == "__main__":
    ...
    def gen_one(test_name, gen_dart_call: Callable, python_func, hi, lo,
                wrap_dart_call: Callable = None):
        for arr_name in named_arrays.keys():
            with NamedTest(f"{test_name} for {arr_name} lo={lo} hi={hi}"):
                for x in range(-2, 9):
    ...
    def gen_hilo_range(test_name: str, gen_dart_call: Callable, python_func):
        hi_lo_ranges = [None] + list(range(0, 10))
        for hi in hi_lo_ranges:
            for lo in hi_lo_ranges:
                gen_one(test_name=test_name, gen_dart_call=gen_dart_call,
